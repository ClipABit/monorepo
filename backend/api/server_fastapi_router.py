...
class ServerFastAPIRouter:
    ...
    def _get_process_video_spawn_fn(self):
        ...
        """
        def spawn_process_video(video_bytes: bytes, filename: str, job_id: str, namespace: str, parent_batch_id: str, user_id: str = None, hashed_identifier: str = ""):
            try:
                if self.processing_service_cls:
                    # Dev combined mode - direct access
                    self.processing_service_cls().process_video_background.spawn(
                        video_bytes, filename, job_id, namespace, parent_batch_id, user_id, hashed_identifier
                    )
    ...
    async def upload(self, request: Request, files: list[UploadFile] = File(default=[]), namespace: str = Form(""), hashed_identifier: str = Form("")):
        """
        ...
        result = await self.upload_handler.handle_upload(files, user_namespace, user_id, hashed_identifier)
