...
class Chunker:
    """
    Video chunking with scene detection and duration constraints.
    
    Algorithm:
    1. Detect scene boundaries using PySceneDetect
    2. Merge scenes shorter than min_duration
    3. Split scenes longer than max_duration
    4. Fallback to fixed chunking if scene detection fails
    """
    ...
    def __init__(
        ...
        """
        Args:
            min_duration: Minimum chunk duration (merge if shorter)
            max_duration: Maximum chunk duration (split if longer)
    ...
    def chunk_video(self, video_path: str, video_id: str) -> List[VideoChunk]:
        """
        Create semantic chunks with duration constraints.
        ...
        Returns:
            List of VideoChunk objects (5-20 seconds each)
        """
    ...
    def _apply_duration_constraints(
        ...
        """
        Merge small scenes and split large ones.
        
        Args:
            scenes: List of (start_timecode, end_timecode) tuples
    ...
    def _fallback_chunking(self, video_path: str, video_id: str) -> List[VideoChunk]:
        """
        Fallback to fixed-duration chunking if scene detection fails.
        Uses the midpoint of min/max duration as chunk size.
        """
        logger.warning("Using fallback fixed-duration chunking")
        
        # Use midpoint of min/max as chunk duration
