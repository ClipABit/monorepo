...
class R2Connector:
    ...
    def _sanitize_filename(self, filename: str) -> str:
        ...
        if not filename:
            raise ValueError(
                "Sanitized filename is empty. Please provide a valid filename."
            )
        return filename
    ...
    def _encode_path(self, bucket_name: str, namespace: str, filename: str) -> str:
        ...
        encoded = (
            base64.urlsafe_b64encode(path.encode("utf-8")).decode("utf-8").rstrip("=")
        )
        return encoded
    ...
    def fetch_video(self, identifier: str) -> Optional[bytes]:
        ...
        try:
            ...
            logger.info(
                f"Fetched video with identifier {identifier}: ({len(video_data)} bytes)"
            )
            return video_data
    ...
    def generate_presigned_url(
        ...
        try:
            ...
            if not object_key:
                logger.warning(
                    f"Cannot generate presigned URL: invalid identifier {identifier}"
                )
                return None
    ...
    def fetch_video_page(
        ...
        Args:
            namespace: Namespace for videos (default "__default__")
            page_size: Number of videos per page (default 20)
            continuation_token: Token from previous page for pagination
            expiration: TTL for presigned URLs in seconds (default 1 hour)

        Returns:
            Tuple[List[dict], Optional[str]]: (videos list, next continuation token or None)
        """
        try:
            if page_size <= 0:
            ...
            items = (
                filtered  # Use all filtered items since we requested exactly page_size
            )
            ...
            for obj in items:
                ...
                try:
                    ...
                    if len(parts) != 2:
                        logger.warning(
                            f"Skipping object with unexpected key format: {object_key}"
                        )
                        continue
                    filename = parts[1]
                    identifier = self._encode_path(
                        self.bucket_name, namespace, filename
                    )
                    ...
                    if url:
                        ...
                        logger.warning(
                            f"Generated empty presigned URL for {object_key}"
                        )
                except Exception as e:
                    logger.error(
                        f"Error processing video {object_key}: {e}", exc_info=True
                    )
    ...
    def list_videos_page(
        ...
        if not cache_hit:
            ...
            if self._url_cache:
                self._url_cache.set_page(
                    namespace, normalized_token, page_size, videos, next_token
                )
        ...
        total_pages = (
            math.ceil(total_videos / page_size) if page_size and total_videos else 0
        )
